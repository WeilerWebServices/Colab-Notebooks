...
import {SocketIoToDap} from './socketio_to_dap';
...
const socketIoHandlers: Array<(SocketIoToPty|SocketIoToDap)> = [];
...
export function run(settings: AppSettings): void {
  ...
  if (settings.debugAdapterMultiplexerPath) {
    socketIoHandlers.push(new SocketIoToDap('/debugger', settings.debugAdapterMultiplexerPath, server));
  }
